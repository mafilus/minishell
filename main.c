...
char *
prompt (struct config_prompt *conf)
{
...
char *
commandline_gets (struct config_gets *conf)
{
...
int
addinternalscmds (struct icmd *cmd)
{
  ...
  {
    void *tmp =
      realloc (internalcmds->commands,
	       (internalcmds->size + 1) * sizeof (struct icmd *));
    if (tmp == NULL)
      {
	return errno;
      }
...
void
addcmd (char *cmd, int (*main) (int argc, char **argv))
{
...
int
run_internalcmd (struct icmds *internalcmds, char *cmd, int argc, char **argv)
{
  int ret = -1;
  int isrun = 0;
...
parse (char *commandline)
{
  ...
  if ((run_internalcmd
       (internalcmds, words.we_wordv[0], words.we_wordc,
	words.we_wordv)) == 0)
...
void
interpreter (struct config_prompt *cnf_prompt, struct config_gets *cnf_gets)
{
  while (1)
    {
      char *cmdline = NULL;
